...
//
// FORKED FROM coworld-ctf, function for function. This is a GENERIC sprite
// compositor: it has no ctf-specific draw call to delete, because every board
// pixel is baked server-side (src/crafter/global.nim) and arrives as sprite
// definitions plus retained-mode object placements. The only edit is the wire
// constants namespace, `window.CTF_WIRE` -> `window.CRAFTER_WIRE`, emitted by
// tools/gen_wire_constants.nim. tests/test_crafter_viewer.nim pins the kept
// procs — `pushFeed`'s signature included (the cogball 0.1.4 latch scar: a
// signature drift threw mid-replay and latched static_replay.js into
// `failed`) — byte-identical against the starter's text.
...
(function() {
  ...
  const CHROME_SPRITE_ID =
    (window.CRAFTER_WIRE && window.CRAFTER_WIRE.chromeSpriteId) || 4090;
