...
##
## THIS MODULE IMPORTS THE SAME `src/crafter/sim.nim` THE SERVER RUNS. That is
...
import
  std/json,
  crafter/[broadcast, replay_runtime, replays, sim]
...
proc crafterLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "crafter_load_replay", cdecl.} =
  try:
    lastError = ""
...
proc crafterInput(data: ptr uint8, length: cint)
    {.exportc: "crafter_input", cdecl.} =
  if runtimeLoaded:
    viewer.applyGlobalViewerMessage(data.bytesFromPointer(int(length)))

proc crafterFrame(): cint {.exportc: "crafter_frame", cdecl.} =
...
proc crafterPacketPointer(): ptr uint8
    {.exportc: "crafter_packet_ptr", cdecl.} =
  if packet.len == 0: nil else: packet[0].addr

proc crafterPacketLength(): cint {.exportc: "crafter_packet_len", cdecl.} =
  cint(packet.len)

proc crafterMismatchTick(): cint {.exportc: "crafter_mismatch_tick", cdecl.} =
...
proc crafterErrorPointer(): ptr uint8 {.exportc: "crafter_error_ptr", cdecl.} =
  if lastError.len == 0: nil else: cast[ptr uint8](lastError[0].addr)

proc crafterErrorLength(): cint {.exportc: "crafter_error_len", cdecl.} =
  cint(lastError.len)

proc crafterStagePointer(): ptr uint8 {.exportc: "crafter_stage_ptr", cdecl.} =
  ## Unlike crafter_error_*, this stays valid after an allocation-failure
  ## abort, so JS can report what the runtime was doing.
  if stageNoteLen == 0: nil else: cast[ptr uint8](stageNote[0].addr)

proc crafterStageLength(): cint {.exportc: "crafter_stage_len", cdecl.} =
  cint(stageNoteLen)
...
when isMainModule and defined(emscripten):
  ...
  ## keeps calling crafter_load_replay / crafter_frame. Unwinding main
