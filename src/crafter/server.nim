...
proc httpHandler(request: Request) {.gcsafe.} =
  ...
  if request.path == HealthPath and request.httpMethod == "GET":
    ...
    echo "crafter: player connected on slot ", slot
    ...
    request.respond(200, headers,
      "<!doctype html><meta charset=\"utf-8\"><title>crafter seat</title>" &
      "<body style=\"background:#14120f;color:#f2e8d8;font:14px system-ui\">" &
      "<h1>crafter</h1><p>Seat " & $slot & " is driven by its policy " &
    ...
    request.respond(200, headers, "crafter server")
...
proc runServerLoop*() =
  ...
  echo "crafter: serving on ", host, ":", port, " seed ", config.seed,
    " variant ", config.variant

  var replayWriter: ReplayWriter
  let replayPath = "/tmp/crafter-" & $config.seed & ".replay"
  try:
    replayWriter = openReplayWriter(replayPath, config.resolvedJson())
  except CatchableError as error:
    echo "crafter: replay writer unavailable: ", error.msg
  ...
  proc drainSockets() =
    ...
    withLock appState.lock:
      ...
      for websocket, text in appState.chatMessages.pairs:
        ...
        echo "crafter: seat ", slot, " registered as ", label, " (",
          (if parsed.isLlm: "llm" else: "scripted"), ")"
  ...
  try:
    while sim.phase != GameOver:
      ...
      if sim.phase == Lobby:
        ...
        if sim.phase == Playing and joinedSlots.len > 0 and not registered:
          ...
          echo "crafter: FATAL — the joined seat sent no register record; ",
        ...
        if wasLobby and sim.phase == Playing and joinedSlots.len == 0:
          ## A seat that never connects does NOT end the episode: the run
          ## plays out to its natural end driven by `forager`, with the seat
          ## marked dead and ONE closed-schema failure payload reported.
          echo "crafter: no seat connected inside the lobby window; the ",
            "run plays out on the forager baseline"
      ...
      ## EDIT 1: the turn boundary. ONE seat, so this is a batch of one, with
      ## a VARIABLE turn length: the tick loop breaks early on a flinch or a
      ## death, and the next turn begins here.
    ...
    echo "crafter: FAULT — ", faultDetail
  ...
  try:
    ...
    echo "crafter: failed to write results: ", error.msg
  try:
    ...
    echo "crafter: failed to write replay: ", error.msg
  try:
    ...
    echo "crafter: failed to write events: ", error.msg

  echo "crafter: episode complete — reason ", sim.endReason, " endRule ",
    sim.endRule, " unlocked ", sim.achievementsUnlocked(), "/",
    AchievementCount, " ticks ", sim.survivalTicks(), " score ", sim.score()
