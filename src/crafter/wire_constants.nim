...
## wasm bundle. Clients read `window.CRAFTER_WIRE` and keep their old
...
const WireConstantsJs* =
  ...
  "window.CRAFTER_WIRE={speeds:[0.5," & jsIntArray(PlaybackSpeeds)[1..^1] &
  ",fps:" & $TargetFps &
  ...
  "};window.CTF_WIRE=window.CTF_WIRE||window.CRAFTER_WIRE;"
  ## The starter's `broadcast_core.js` reads `window.CTF_WIRE` for the chrome
  ## sprite id. It is kept as an ALIAS of the crafter block rather than
