## The binary `COWLDCRF` replay: the codec wrapper, playback keyframes, the
...
## (`COWLDCTF` -> `COWLDCRF`) plus the retarget of the per-tick input stream:
...
const
  ...
  LullLeadTicks* = 12
  MinLullTicks* = 40
    ## A lull is 40 consecutive ticks with no `achievement`, `collect`,
    ## `craft`, `place`, `kill`, `hurt` or `nightfall` event and no change in
    ## the cog's cell — the design note's definition, in ticks.
  LullSpeedBoost* = 8
  MaxLullTicksPerFrame* = 64
  SeekTicksPerFrame* = 240
  CrafterReplayMagic* = "COWLDCRF"
  CrafterReplayFormatVersion = 1'u16
  CrafterReplaySpec* = ReplaySpec(
    magic: CrafterReplayMagic,
    formatVersion: CrafterReplayFormatVersion,
    gameName: GameName,
...
proc openReplayWriter*(path: string, configJson: string): ReplayWriter =
  replayCodec.openReplayWriter(path, configJson, CrafterReplaySpec)

proc parseReplayBytes*(bytes: string): ReplayData =
  replayCodec.parseReplayBytes(bytes, CrafterReplaySpec)

proc loadReplay*(path: string): ReplayData =
  replayCodec.loadReplay(path, CrafterReplaySpec)
...
proc pushControlEvents*(sim: var SimServer, record: JsonNode) =
  ...
  of "directive":
    ...
    sim.pending.add(SimEvent(kind: evPlan, tick: sim.tickCount,
      ...
      m: record{"dropped"}.getInt(), b: record{"interrupted"}.getStr()))
  of "fallback":
...
proc stepReplay*(replay: var ReplayPlayer, sim: var SimServer) =
  ...
  ##
  ## A TURN IS `turnTicks` TICKS and the sim only steps while a turn is
  ## active, so a recorded stream that runs out of `directive` records leaves
  ## the tick where it is. Stopping playback there is the only safe answer: a
  ## loop that kept calling this would spin forever.
...
proc scanLead(sim: SimServer): seq[int] =
  ## The cumulative achievement count (0..22) — the series the achievement
  ## sparkline plots, with the night spans shaded behind it.
  @[sim.achievementsUnlocked()]
...
proc advanceReplayScan*(replay: var ReplayPlayer, maxTicks: int) =
  ...
  while stepsLeft > 0 and scan.builder.playing and
    ...
    let beforeTick = scan.sim.tickCount
    try:
      scan.builder.stepReplay(scan.sim)
      if scan.sim.tickCount == beforeTick:
    ...
    for event in stepBeats:
      ...
      ## A lull is a run of consecutive ticks with no achievement, collect,
      ## craft, place, kill, hurt or nightfall event and no change in the
      ## cog's cell.
      if kind in {evAchievement, evCollect, evCraft, evPlace, evKill, evHurt,
                  evNightfall, evDaybreak, evDeath, evEnd}:
        beatHere = true
    let cell = idx(scan.sim.cog.x, scan.sim.cog.y)
...
proc replayStepBudget*(replay: ReplayPlayer, tick: int): int =
  ## Returns how many ticks playback may advance this frame: the chosen
  ## speed, boosted inside a lull while skip-lulls is on. At 1/2x a tick is
  ## spent only every other frame (halfPhase parity).
