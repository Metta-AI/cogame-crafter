## The crafter player container: a policy is just a prompt.
##
...
##
## A seat that sets neither is `forager`. To field your own policy, reuse
## this image and set PLAYER_PROMPT:
##
##   coworld upload-policy <crafter-image> --name my-crafter \
...
proc registrationBlob(prompt, scripted, policy: string): string =
  ...
  ## an LLM seat, so the server can tell "no baseline named" from "forager
  ## named explicitly".
...
when isMainModule:
  ...
  echo "crafter player: kind=",
    (if prompt.len > 0: "llm" else: "scripted"),
    " baseline=", (if scripted.len > 0: scripted else: "forager"),
    " label=", label
  ...
  proc dial(attempts: int): WebSocket =
    ...
    for attempt in 0 ..< attempts:
      try:
        ...
        if attempt == 0:
          echo "crafter player: game not listening yet (", error.msg,
            "); retrying"
  ...
  if socket == nil:
    quit("crafter player: game never accepted a connection", 1)
  echo "crafter player: connected"
  ...
  while true:
    ...
    try:
      ...
      echo "crafter player: socket closed (", error.msg, ")"
    ...
    if sessionFrames == 0 or reconnects >= ReconnectAttempts:
      break
    inc reconnects
    echo "crafter player: re-dialling the seat (attempt ", reconnects, ")"
    socket = dial(ReconnectAttempts)
    if socket == nil:
      echo "crafter player: game is no longer listening, exiting cleanly"
      break
    echo "crafter player: reconnected, re-registering"
  quit(0)
